...
STAGE=<< parameters.stage >> yarn build:mac
cd ../../..
`
    }),
    new CircleCI.commands.Run({
      ...
      command: `
cd ./packages/<< parameters.package_path >>
AWS_PROFILE=tooling aws s3 sync ./dist/ "s3://vysta-devops/artifacts/<< parameters.stage >>/<< parameters.package_path >>/mac-prerelease" --include='*.dmg';
AWS_PROFILE=tooling aws s3 sync ./dist/ "s3://vysta-devops/artifacts/<< parameters.stage >>/<< parameters.package_path >>/mac" --include='*.dmg';
cd ../../..
`
    }),
