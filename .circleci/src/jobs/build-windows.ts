...
$env:STAGE = '<< parameters.stage >>'; yarn build:windows
cd ..\\\\..\\\\..
`
    }),
    new CircleCI.commands.Run({
      ...
      command: `
cd .\\\\packages\\\\<< parameters.package_path >>
$env:AWS_PROFILE = 'tooling'; aws s3 sync .\\\\dist\\\\ "s3://vysta-devops/artifacts/<< parameters.stage >>/<< parameters.package_path >>/windows-prerelease" --include='*.exe';
$env:AWS_PROFILE = 'tooling'; aws s3 sync .\\\\dist\\\\ "s3://vysta-devops/artifacts/<< parameters.stage >>/<< parameters.package_path >>/windows" --include='*.exe';
cd ..\\\\..\\\\..
`
    })
  ],
