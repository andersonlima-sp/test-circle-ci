...
`,
    ...
    SageCICommands.SaveBuildCache.reuseCommand({ suffix: '-win-v1' }),
    new CircleCI.commands.Run({
      name: 'Build Electron App',
      command: `
cd .\\\\packages\\\\<< parameters.package_path >>
$env:STAGE = '<< parameters.stage >>'; yarn build:<< parameters.application_type >>:windows
cd ..\\\\..\\\\..
`
    }),
    new CircleCI.commands.Run({
      ...
      command: `
cd .\\\\packages\\\\<< parameters.package_path >>
$env:AWS_PROFILE = 'tooling'; aws s3 sync .\\\\dist\\\\ "s3://vysta-devops/artifacts/<< parameters.stage >>/<< parameters.package_path >>/windows-<< parameters.application_type >>" --include='*.exe';
cd ..\\\\..\\\\..
`
    })
  ],
